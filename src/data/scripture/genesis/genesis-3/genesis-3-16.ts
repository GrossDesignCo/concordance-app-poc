import { Verse } from '@/types';

export const genesis_3_16: Verse = {
  meta: {
    book: 'Genesis',
    chapter: 3,
    verse: 16,
  },
  words: [
    {
      hebrew: 'אֶל',
      transliteration: 'el',
      englishLiteral: 'To',
      englishNatural: 'To',
      root: 'el',
      order: {
        hebrew: 1,
        english: 1,
      },
      morphology: {
        type: 'preposition'
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'הָאִשָּׁה',
      transliteration: 'haIshah',
      englishLiteral: 'the-woman',
      englishNatural: 'the woman',
      root: 'ishah',
      prefixes: ['ha'],
      order: {
        hebrew: 2,
        english: 2,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        type: 'noun'
      },
    },
    {
      hebrew: 'אָמַר',
      transliteration: 'amar',
      englishLiteral: 'he-said',
      englishNatural: 'he said',
      root: 'amar',
      order: {
        hebrew: 3,
        english: 3,
      },
      morphology: {
        person: '3rd',
        gender: 'masculine',
        number: 'singular',
        tense: 'perfect',
        stem: 'qal',
        type: 'verb'
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'הַרְבָּה',
      transliteration: 'harbah',
      englishLiteral: 'Multiplying',
      englishNatural: 'Multiplying',
      root: 'harbah',
      order: {
        hebrew: 4,
        english: 4,
      },
      morphology: {
        type: 'adverb'
      },
      grammarPrefix: {
        englishLiteral: '"',
        englishNatural: '"',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'אַרְבֶּה',
      transliteration: 'arbeh',
      englishLiteral: 'I-will_multiply',
      englishNatural: 'I will multiply',
      root: 'rabah',
      order: {
        hebrew: 5,
        english: 5,
      },
      morphology: {
        person: '1st',
        number: 'singular',
        tense: 'imperfect',
        stem: 'hiphil',
        type: 'verb'
      },
    },
    {
      hebrew: 'עִצְּבוֹנֵךְ',
      transliteration: 'itzvonek',
      englishLiteral: 'pain-your',
      englishNatural: 'your pain',
      root: 'etzev',
      suffixes: ['ekh'],
      order: {
        hebrew: 6,
        english: 6,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        type: 'noun'
      },
    },
    {
      hebrew: 'וְהֵרֹנֵךְ',
      transliteration: 'veHeronekh',
      englishLiteral: 'and-pregnancy-your',
      englishNatural: 'and your pregnancy',
      root: 'heron',
      prefixes: ['ve'],
      suffixes: ['ekh'],
      order: {
        hebrew: 7,
        english: 7,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        type: 'noun'
      },
      grammarSuffix: {
        englishLiteral: ';',
        englishNatural: ';',
      },
    },
    {
      hebrew: 'בְּעֶצֶב',
      transliteration: 'beEtzev',
      englishLiteral: 'in-pain',
      englishNatural: 'in pain',
      root: 'etzev',
      prefixes: ['be'],
      order: {
        hebrew: 8,
        english: 8,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        type: 'noun'
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'תֵּלְדִי',
      transliteration: 'teldi',
      englishLiteral: 'you-will_birth',
      englishNatural: 'you will birth',
      root: 'yalad',
      prefixes: ['te'],
      order: {
        hebrew: 9,
        english: 9,
      },
      morphology: {
        person: '2nd',
        gender: 'feminine',
        number: 'singular',
        tense: 'imperfect',
        stem: 'qal',
        type: 'verb'
      },
    },
    {
      hebrew: 'בָנִים',
      transliteration: 'banim',
      englishLiteral: 'sons',
      englishNatural: 'sons',
      root: 'ben',
      suffixes: ['im'],
      order: {
        hebrew: 10,
        english: 10,
      },
      morphology: {
        gender: 'masculine',
        number: 'plural',
        type: 'noun'
      },
      grammarSuffix: {
        englishLiteral: ';',
        englishNatural: ';',
      },
    },
    {
      hebrew: 'וְאֶל',
      transliteration: 'veEl',
      englishLiteral: 'and-to',
      englishNatural: 'and to',
      root: 'el',
      prefixes: ['ve'],
      order: {
        hebrew: 11,
        english: 11,
      },
      morphology: {
        type: 'preposition'
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'אִישֵׁךְ',
      transliteration: 'ishekh',
      englishLiteral: 'man-your',
      englishNatural: 'your man',
      root: 'ish',
      suffixes: ['ekh'],
      order: {
        hebrew: 12,
        english: 12,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        type: 'noun'
      },
    },
    {
      hebrew: 'תְּשׁוּקָתֵךְ',
      transliteration: 'teshuqatek',
      englishLiteral: 'attraction-your',
      englishNatural: 'your attraction',
      root: 'teshuqah',
      suffixes: ['ekh'],
      order: {
        hebrew: 13,
        english: 13,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        type: 'noun'
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'וְהוּא',
      transliteration: 'veHu',
      englishLiteral: 'and-he',
      englishNatural: 'and he',
      root: 'hu',
      prefixes: ['ve'],
      order: {
        hebrew: 14,
        english: 14,
      },
      morphology: {
        type: 'pronoun'
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'יִמְשָׁל',
      transliteration: 'yimshal',
      englishLiteral: 'he-will_rule',
      englishNatural: 'will rule',
      root: 'mashal',
      prefixes: ['yi'],
      order: {
        hebrew: 15,
        english: 15,
      },
      morphology: {
        person: '3rd',
        gender: 'masculine',
        number: 'singular',
        tense: 'imperfect',
        stem: 'qal',
        type: 'verb'
      },
    },
    {
      hebrew: 'בָּךְ',
      transliteration: 'bakh',
      englishLiteral: 'in-you',
      englishNatural: 'in you',
      root: 'be',
      suffixes: ['ekh'],
      order: {
        hebrew: 16,
        english: 16,
      },
      morphology: {
        type: 'preposition'
      },
      grammarSuffix: {
        englishLiteral: '."',
        englishNatural: '."',
      },
    },
  ],
  expectedTranslations: {
    hebrew: 'אֶל הָאִשָּׁה אָמַר הַרְבָּה אַרְבֶּה עִצְּבוֹנֵךְ וְהֵרֹנֵךְ בְּעֶצֶב תֵּלְדִי בָנִים וְאֶל אִישֵׁךְ תְּשׁוּקָתֵךְ וְהוּא יִמְשָׁל בָּךְ',
    transliteration: 'el haIshah amar harbah arbeh itzvonek veHeronekh beEtzev teldi banim veEl ishekh teshuqatek veHu yimshal bakh',
    englishLiteral: 'To the-woman he-said, "Multiplying, I-will_multiply pain-your and-pregnancy-your; in-pain you-will_birth sons; and-to man-your attraction-your, and-he he-will_rule in-you."',
    englishNatural: 'To the woman he said, "Multiplying, I will multiply your pain and your pregnancy; in pain you will birth sons; and to your man your attraction, and he will rule in you."',
    kjv: 'Unto the woman he said, I will greatly multiply thy sorrow and thy conception; in sorrow thou shalt bring forth children; and thy desire shall be to thy husband, and he shall rule over thee.',
    lastReviewed: { name: 'Matt Gross', date: '2025-06-10' },
  },
}; 