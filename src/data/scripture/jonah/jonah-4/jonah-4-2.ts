import { Verse } from '@/types';

export const jonah_4_2: Verse = {
  meta: {
    book: 'Jonah',
    chapter: 4,
    verse: 2,
  },
  words: [
    {
      hebrew: 'וַיִּתְפַּלֵּל',
      transliteration: 'vaYitpallel',
      englishLiteral: 'And-prayed',
      englishNatural: 'And he prayed',
      root: 'palal',
      prefixes: ['va'],
      order: {
        hebrew: 1,
        english: 1,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        person: '3rd',
        tense: 'imperfect',
        stem: 'hithpael',
        type: 'verb',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'אֶל־',
      transliteration: 'el-',
      englishLiteral: 'to-',
      englishNatural: 'to',
      root: 'el',
      order: {
        hebrew: 2,
        english: 2,
      },
      morphology: {
        type: 'preposition',
      },
    },
    {
      hebrew: 'יְהוָה',
      transliteration: 'YHWH',
      englishLiteral: 'He_Who_Is',
      englishNatural: 'He-Who-Is',
      root: 'yhwh',
      order: {
        hebrew: 3,
        english: 3,
      },
      morphology: {
        type: 'noun',
      },
    },
    {
      hebrew: 'וַיֹּאמַר',
      transliteration: 'vaYomer',
      englishLiteral: 'and-said',
      englishNatural: 'and said',
      root: 'amar',
      prefixes: ['va'],
      order: {
        hebrew: 4,
        english: 4,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        person: '3rd',
        tense: 'imperfect',
        stem: 'qal',
        type: 'verb',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'אָנָּה',
      transliteration: 'anna',
      englishLiteral: 'please',
      englishNatural: 'Please',
      root: 'anna',
      order: {
        hebrew: 5,
        english: 5,
      },
      morphology: {
        type: 'particle',
      },
      grammarPrefix: {
        englishLiteral: '"',
        englishNatural: '"',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'יְהוָה',
      transliteration: 'YHWH',
      englishLiteral: 'He_Who_Is',
      englishNatural: 'He-Who-Is',
      root: 'yhwh',
      order: {
        hebrew: 6,
        english: 6,
      },
      morphology: {
        type: 'noun',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'הֲלוֹא־',
      transliteration: 'halo-',
      englishLiteral: 'is-not-',
      englishNatural: 'was not',
      root: 'lo',
      prefixes: ['ha'],
      order: {
        hebrew: 7,
        english: 7,
      },
      morphology: {
        type: 'adverb',
      },
    },
    {
      hebrew: 'זֶה',
      transliteration: 'zeh',
      englishLiteral: 'this',
      englishNatural: 'this',
      root: 'zeh',
      order: {
        hebrew: 8,
        english: 8,
      },
      morphology: {
        type: 'pronoun',
      },
    },
    {
      hebrew: 'דְבָרִי',
      transliteration: 'devari',
      englishLiteral: 'word-my',
      englishNatural: 'my word',
      root: 'davar',
      suffixes: ['i_possessive'],
      order: {
        hebrew: 9,
        english: 9,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        type: 'noun',
      },
    },
    {
      hebrew: 'עַד־',
      transliteration: 'ad-',
      englishLiteral: 'until-',
      englishNatural: 'until',
      root: 'ad',
      order: {
        hebrew: 10,
        english: 10,
      },
      morphology: {
        type: 'preposition',
      },
    },
    {
      hebrew: 'הֱיוֹתִי',
      transliteration: 'heyoti',
      englishLiteral: 'being-my',
      englishNatural: 'I was',
      root: 'hayah',
      suffixes: ['i_possessive'],
      order: {
        hebrew: 11,
        english: 11,
      },
      morphology: {
        number: 'singular',
        person: '1st',
        tense: 'infinitive_construct',
        stem: 'qal',
        type: 'verb',
      },
    },
    {
      hebrew: 'עַל־',
      transliteration: 'al-',
      englishLiteral: 'over-',
      englishNatural: 'over',
      root: 'al',
      order: {
        hebrew: 12,
        english: 12,
      },
      morphology: {
        type: 'preposition',
      },
    },
    {
      hebrew: 'אַדְמָתִי',
      transliteration: 'admati',
      englishLiteral: 'soil-my',
      englishNatural: 'my soil',
      root: 'adamah',
      suffixes: ['i_possessive'],
      order: {
        hebrew: 13,
        english: 13,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        type: 'noun',
      },
      grammarSuffix: {
        englishLiteral: '?',
        englishNatural: '?',
      },
    },
    {
      hebrew: 'עַל־',
      transliteration: 'al-',
      englishLiteral: 'over-',
      englishNatural: 'moreover',
      root: 'al',
      order: {
        hebrew: 14,
        english: 15,
      },
      morphology: {
        type: 'preposition',
      },
      lineBreaksBefore: {
        hebrew: 1,
      },
    },
    {
      hebrew: 'כֵּן',
      transliteration: 'ken',
      englishLiteral: 'so',
      englishNatural: 'So',
      root: 'ken',
      order: {
        hebrew: 15,
        english: 14,
      },
      morphology: {
        type: 'adverb',
      },
      lineBreaksBefore: {
        english: 1,
      },
    },
    {
      hebrew: 'קִדַּמְתִּי',
      transliteration: 'qidamti',
      englishLiteral: 'I-preceded',
      englishNatural: 'I preceded',
      root: 'qidam',
      suffixes: ['ti'],
      order: {
        hebrew: 16,
        english: 16,
      },
      morphology: {
        number: 'singular',
        person: '1st',
        tense: 'perfect',
        stem: 'piel',
        type: 'verb',
      },
    },
    {
      hebrew: 'לִבְרֹחַ',
      transliteration: 'livroch',
      englishLiteral: 'to-flee',
      englishNatural: 'to flee to',
      root: 'barach',
      prefixes: ['li'],
      order: {
        hebrew: 17,
        english: 17,
      },
      morphology: {
        type: 'verb',
        tense: 'infinitive_construct',
        stem: 'qal',
      },
    },
    {
      hebrew: 'תַּרְשִׁישָׁה',
      transliteration: 'tarshishah',
      englishLiteral: 'Breaking (Tarshish)',
      englishNatural: 'Breaking (Tarshish)',
      root: 'tarshish',
      suffixes: ['ah'],
      order: {
        hebrew: 18,
        english: 18,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        type: 'noun',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'כִּי',
      transliteration: 'ki',
      englishLiteral: 'that',
      englishNatural: 'that',
      root: 'ki',
      order: {
        hebrew: 19,
        english: 19,
      },
      morphology: {
        type: 'relative',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'יָדַעְתִּי',
      transliteration: 'yadati',
      englishLiteral: 'I-knew',
      englishNatural: 'I knew',
      root: 'yada',
      suffixes: ['ti'],
      order: {
        hebrew: 20,
        english: 20,
      },
      morphology: {
        number: 'singular',
        person: '1st',
        tense: 'perfect',
        stem: 'qal',
        type: 'verb',
      },
    },
    {
      hebrew: 'כִּי־',
      transliteration: 'ki-',
      englishLiteral: 'that-',
      englishNatural: 'that',
      root: 'ki',
      order: {
        hebrew: 21,
        english: 21,
      },
      morphology: {
        type: 'relative',
      },
    },
    {
      hebrew: 'אַתָּה',
      transliteration: 'atah',
      englishLiteral: 'you',
      englishNatural: 'you are',
      root: 'atah',
      order: {
        hebrew: 22,
        english: 22,
      },
      morphology: {
        type: 'pronoun',
      },
    },
    {
      hebrew: 'אֵל־',
      transliteration: 'el-',
      englishLiteral: 'God-',
      englishNatural: 'God',
      root: 'el',
      order: {
        hebrew: 23,
        english: 23,
      },
      morphology: {
        type: 'noun',
      },
    },
    {
      hebrew: 'חַנּוּן',
      transliteration: 'chanun',
      englishLiteral: 'gracious',
      englishNatural: 'gracious',
      root: 'chanun',
      order: {
        hebrew: 24,
        english: 24,
      },
      morphology: {
        type: 'adjective',
      },
    },
    {
      hebrew: 'וְרַחוּם',
      transliteration: 'veRachum',
      englishLiteral: 'and-compassionate',
      englishNatural: 'and compassionate',
      root: 'rachum',
      prefixes: ['ve'],
      order: {
        hebrew: 25,
        english: 25,
      },
      morphology: {
        type: 'adjective',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'אֶרֶךְ',
      transliteration: 'erekh',
      englishLiteral: 'long',
      englishNatural: 'long',
      root: 'erekh',
      order: {
        hebrew: 26,
        english: 26,
      },
      morphology: {
        type: 'adjective',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'אַפַּיִם',
      transliteration: 'apayim',
      englishLiteral: 'nostrils',
      englishNatural: 'of nostrils',
      root: 'aph',
      order: {
        hebrew: 27,
        english: 27,
      },
      morphology: {
        gender: 'masculine',
        number: 'dual',
        type: 'noun',
      },
    },
    {
      hebrew: 'וְרַב־',
      transliteration: 'veRav-',
      englishLiteral: 'and-great-',
      englishNatural: 'and great',
      root: 'rav',
      prefixes: ['ve'],
      order: {
        hebrew: 28,
        english: 28,
      },
      morphology: {
        type: 'adjective',
      },
    },
    {
      hebrew: 'חֶסֶד',
      transliteration: 'chesed',
      englishLiteral: 'steadfast_love',
      englishNatural: 'of steadfast-love',
      root: 'chesed',
      order: {
        hebrew: 29,
        english: 29,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        type: 'noun',
      },
    },
    {
      hebrew: 'וְנִחָם',
      transliteration: 'veNicham',
      englishLiteral: 'and-comforting',
      englishNatural: 'and comforting',
      root: 'nacham',
      prefixes: ['ve'],
      order: {
        hebrew: 30,
        english: 30,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        person: '3rd',
        tense: 'participle',
        stem: 'niphal',
        type: 'verb',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'עַל־',
      transliteration: 'al-',
      englishLiteral: 'over-',
      englishNatural: 'over',
      root: 'al',
      order: {
        hebrew: 31,
        english: 31,
      },
      morphology: {
        type: 'preposition',
      },
    },
    {
      hebrew: 'הָרָעָה',
      transliteration: 'haRaah',
      englishLiteral: 'the-bad',
      englishNatural: 'the bad',
      root: 'ra',
      prefixes: ['ha'],
      order: {
        hebrew: 32,
        english: 32,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        type: 'adjective',
      },
      grammarSuffix: {
        englishLiteral: '.',
        englishNatural: '.',
      },
      lineBreaksAfter: 1,
    },
  ],
  expectedTranslations: {
    hebrew:
      'וַיִּתְפַּלֵּל אֶל־יְהוָה וַיֹּאמַר אָנָּה יְהוָה הֲלוֹא־זֶה דְבָרִי עַד־הֱיוֹתִי עַל־אַדְמָתִי עַל־כֵּן קִדַּמְתִּי לִבְרֹחַ תַּרְשִׁישָׁה כִּי יָדַעְתִּי כִּי־אַתָּה אֵל־חַנּוּן וְרַחוּם אֶרֶךְ אַפַּיִם וְרַב־חֶסֶד וְנִחָם עַל־הָרָעָה',
    transliteration:
      'vaYitpallel el-YHWH vaYomer anna YHWH halo-zeh devari ad-heyoti al-admati al-ken qidamti livroch tarshishah ki yadati ki-atah el-chanun veRachum erekh apayim veRav-chesed veNicham al-haRaah',
    englishLiteral:
      'And-prayed to- He_Who_Is and-said, "please He_Who_Is, is-not- this word-my until- being-my over- soil-my? over- so I-preceded to-flee Breaking (Tarshish), that I-knew that- you God- gracious and-compassionate, long nostrils and-great- steadfast_love and-comforting over- the-bad.',
    englishNatural:
      'And he prayed to He-Who-Is and said, "Please He-Who-Is, was not this my word until I was over my soil? So moreover I preceded to flee to Breaking (Tarshish), that I knew that you are God gracious and compassionate, long of nostrils and great of steadfast-love and comforting over the bad.',
    kjv: 'And he prayed unto the LORD, and said, I pray thee, O LORD, was not this my saying, when I was yet in my country? Therefore I fled before unto Tarshish: for I knew that thou art a gracious God, and merciful, slow to anger, and of great kindness, and repentest thee of the evil.',
    lastReviewed: { name: 'Matt Gross', date: '2025-10-19' },
  },
};
