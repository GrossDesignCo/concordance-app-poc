import { Verse } from '@/types';

export const jonah_1_3: Verse = {
  meta: {
    book: 'Jonah',
    chapter: 1,
    verse: 3,
  },
  words: [
    {
      hebrew: 'וַיָּקָם',
      transliteration: 'vaYaqam',
      englishLiteral: 'And-stood_up',
      englishNatural: 'stood-up',
      root: 'qum',
      prefixes: ['va'],
      order: {
        hebrew: 1,
        english: 2,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        person: '3rd',
        tense: 'imperfect',
        stem: 'qal',
        type: 'verb',
      },
      grammarSuffix: {
        englishNatural: ',',
      },
      lineBreaksBefore: {
        hebrew: 1,
      },
    },
    {
      hebrew: 'יוֹנָה',
      transliteration: 'Yonah',
      englishLiteral: 'Dove (Jonah)',
      englishNatural: 'And Dove (Jonah)',
      root: 'yonah',
      order: {
        hebrew: 2,
        english: 1,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        type: 'noun',
      },
      grammarSuffix: {
        englishLiteral: ',',
      },
      lineBreaksBefore: {
        english: 1,
      },
    },
    {
      hebrew: 'לִבְרֹחַ',
      transliteration: 'livroach',
      englishLiteral: 'to-flee',
      englishNatural: 'to flee to',
      root: 'barach',
      prefixes: ['li'],
      order: {
        hebrew: 3,
        english: 3,
      },
      morphology: {
        type: 'verb',
        tense: 'infinitive_construct',
        stem: 'qal',
      },
    },
    {
      hebrew: 'תַּרְשִׁישָׁה',
      transliteration: 'Tarshishah',
      englishLiteral: 'Gold-Stone (Tarshish)',
      englishNatural: 'Gold-Stone (Tarshish)',
      root: 'tarshish',
      suffixes: ['ah'],
      order: {
        hebrew: 4,
        english: 4,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        type: 'noun',
        direction: 'directive',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'מִלִּפְנֵי',
      transliteration: 'millifney',
      englishLiteral: 'from-the-face-of',
      englishNatural: 'from the face of',
      root: 'millifney',
      order: {
        hebrew: 5,
        english: 5,
      },
      morphology: {
        type: 'preposition',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'יְהוָה',
      transliteration: 'YHWH',
      englishLiteral: 'He_Who_Is',
      englishNatural: 'He-Who-Is',
      root: 'yhwh',
      order: {
        hebrew: 6,
        english: 6,
      },
      morphology: {
        type: 'noun',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'וַיֵּרֶד',
      transliteration: 'vaYered',
      englishLiteral: 'and-went_down',
      englishNatural: 'and went-down to',
      root: 'yarad',
      prefixes: ['va'],
      order: {
        hebrew: 7,
        english: 7,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        person: '3rd',
        tense: 'imperfect',
        stem: 'qal',
        type: 'verb',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'יָפוֹ',
      transliteration: 'Yafo',
      englishLiteral: 'Beautiful (Joppa)',
      englishNatural: 'Beautiful (Joppa)',
      root: 'yafo',
      order: {
        hebrew: 8,
        english: 8,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        type: 'noun',
      },
      grammarSuffix: {
        englishLiteral: ';',
        englishNatural: ';',
      },
    },
    {
      hebrew: 'וַיִּמְצָא',
      transliteration: 'vaYimtza',
      englishLiteral: 'and-found',
      englishNatural: 'and he found',
      root: 'matza',
      prefixes: ['va'],
      order: {
        hebrew: 9,
        english: 9,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        person: '3rd',
        tense: 'imperfect',
        stem: 'qal',
        type: 'verb',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'אָנִיָּה',
      transliteration: 'aniyah',
      englishLiteral: 'ship',
      englishNatural: 'a ship',
      root: 'aniyah',
      order: {
        hebrew: 10,
        english: 10,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        type: 'noun',
      },
    },
    {
      hebrew: 'בָּאָה',
      transliteration: 'baah',
      englishLiteral: 'going',
      englishNatural: 'going to',
      root: 'baah',
      order: {
        hebrew: 11,
        english: 11,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        tense: 'participle',
        stem: 'qal',
        type: 'verb',
      },
    },
    {
      hebrew: 'תַרְשִׁישׁ',
      transliteration: 'Tarshish',
      englishLiteral: 'Gold-Stone (Tarshish)',
      englishNatural: 'Gold-Stone (Tarshish)',
      root: 'tarshish',
      order: {
        hebrew: 12,
        english: 12,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        type: 'noun',
      },
      grammarSuffix: {
        englishLiteral: ';',
        englishNatural: ';',
      },
    },
    {
      hebrew: 'וַיִּתֵּן',
      transliteration: 'vaYitten',
      englishLiteral: 'and-gave',
      englishNatural: 'and he paid',
      root: 'natan',
      prefixes: ['va'],
      order: {
        hebrew: 13,
        english: 13,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        person: '3rd',
        tense: 'imperfect',
        stem: 'qal',
        type: 'verb',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'שְׂכָרָהּ',
      transliteration: 'sekhorah',
      englishLiteral: 'fare-her',
      englishNatural: 'the fare of it',
      root: 'sakhar',
      suffixes: ['ah'],
      order: {
        hebrew: 14,
        english: 14,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        type: 'noun',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'וַיֵּרֶד',
      transliteration: 'vaYered',
      englishLiteral: 'and-went_down',
      englishNatural: 'and went-down',
      root: 'yarad',
      prefixes: ['va'],
      order: {
        hebrew: 15,
        english: 15,
      },
      morphology: {
        gender: 'masculine',
        number: 'singular',
        person: '3rd',
        tense: 'imperfect',
        stem: 'qal',
        type: 'verb',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'בָּהּ',
      transliteration: 'bah',
      englishLiteral: 'in-her',
      englishNatural: 'into it',
      root: 'be',
      suffixes: ['ah'],
      order: {
        hebrew: 16,
        english: 16,
      },
      morphology: {
        type: 'preposition',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'לָבוֹא',
      transliteration: 'lavo',
      englishLiteral: 'to-go',
      englishNatural: 'to go',
      root: 'bo',
      prefixes: ['la'],
      order: {
        hebrew: 17,
        english: 17,
      },
      morphology: {
        type: 'verb',
        tense: 'infinitive_construct',
        stem: 'qal',
      },
    },
    {
      hebrew: 'עִמָּהֶם',
      transliteration: 'immahhem',
      englishLiteral: 'with-them',
      englishNatural: 'with them to',
      root: 'immahhem',
      order: {
        hebrew: 18,
        english: 18,
      },
      morphology: {
        type: 'preposition',
      },
    },
    {
      hebrew: 'תַּרְשִׁישָׁה',
      transliteration: 'Tarshishah',
      englishLiteral: 'Gold-Stone (Tarshish)',
      englishNatural: 'Gold-Stone (Tarshish)',
      root: 'tarshish',
      suffixes: ['ah'],
      order: {
        hebrew: 19,
        english: 19,
      },
      morphology: {
        gender: 'feminine',
        number: 'singular',
        type: 'noun',
        direction: 'directive',
      },
      grammarSuffix: {
        englishLiteral: ',',
        englishNatural: ',',
      },
    },
    {
      hebrew: 'מִלִּפְנֵי',
      transliteration: 'millifney',
      englishLiteral: 'from-the-face-of',
      englishNatural: 'from the face of',
      root: 'millifney',
      order: {
        hebrew: 20,
        english: 20,
      },
      morphology: {
        type: 'preposition',
      },
      lineBreaksBefore: 1,
    },
    {
      hebrew: 'יְהוָה',
      transliteration: 'YHWH',
      englishLiteral: 'He_Who_Is',
      englishNatural: 'He-Who-Is',
      root: 'yhwh',
      order: {
        hebrew: 21,
        english: 21,
      },
      morphology: {
        type: 'noun',
      },
      grammarSuffix: {
        englishLiteral: '.',
        englishNatural: '.',
      },
      lineBreaksAfter: 1,
    },
  ],
  expectedTranslations: {
    hebrew:
      'וַיָּקָם יוֹנָה לִבְרֹחַ תַּרְשִׁישָׁה מִלִּפְנֵי יְהוָה וַיֵּרֶד יָפוֹ וַיִּמְצָא אָנִיָּה בָּאָה תַרְשִׁישׁ וַיִּתֵּן שְׂכָרָהּ וַיֵּרֶד בָּהּ לָבוֹא עִמָּהֶם תַּרְשִׁישָׁה מִלִּפְנֵי יְהוָה',
    transliteration:
      'vaYaqam Yonah livroach Tarshishah millifney YHWH vaYered Yafo vaYimtza aniyah baah Tarshish vaYitten sekhorah vaYered bah lavo immahhem Tarshishah millifney YHWH',
    englishLiteral:
      'And-stood_up Dove (Jonah), to-flee Gold-Stone (Tarshish), from-the-face-of He_Who_Is, and-went_down Beautiful (Joppa); and-found ship going Gold-Stone (Tarshish); and-gave fare-her, and-went_down in-her, to-go with-them Gold-Stone (Tarshish), from-the-face-of He_Who_Is.',
    englishNatural:
      'And Dove (Jonah) stood-up, to flee to Gold-Stone (Tarshish), from the face of He-Who-Is, and went-down to Beautiful (Joppa); and he found a ship going to Gold-Stone (Tarshish); and he paid the fare of it, and went-down into it, to go with them to Gold-Stone (Tarshish), from the face of He-Who-Is.',
    kjv: 'But Jonah rose up to flee unto Tarshish from the presence of the LORD, and went down to Joppa; and he found a ship going to Tarshish: so he paid the fare thereof, and went down into it, to go with them unto Tarshish from the presence of the LORD.',
    lastReviewed: { name: 'Matt Gross', date: '2025-11-23' },
  },
};
